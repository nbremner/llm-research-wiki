...
Inclusive HR systems are personnel systems designed so work definitions, criteria, evidence, and decisions include the experiences of different employee groups. The core issue is not only whether a downstream selection or performance process produces adverse impact; it is whether the system's upstream assumptions about work, performance, and merit were inclusive in the first place.

[[2022-strah-diversity-issues-job-analysis]] argues that job analysis can either support or undermine inclusion across HR systems. When a job analysis captures only majority-group norms, relies on biased archival language, suppresses underrepresented SME voice, or averages away meaningful subgroup differences, recruitment, selection, performance evaluation, promotion, and compensation can all become built around an incomplete performance domain.

The paper's practical contribution is to move inclusion upstream into system design. Inclusive HR systems require attention to organizational climate, SME participation, confidentiality, rapport, tokenism in panels, bias in job materials, differential task assignment, and the possibility that underrepresented workers perform important work that is structurally undervalued or less visible.
