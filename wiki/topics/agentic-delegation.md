...
Agentic delegation is the handoff of decision authority or task execution to an AI agent rather than only receiving advice, critique, co-production help, or generated material. It is one mode in a broader human-AI collaboration typology: advice-taking, coaching/feedback, stewardship, co-production, proactivity, and delegation differ by who initiates, who executes, who audits, and where accountability sits.

The key question is not whether the agent can perform well in isolation; it is whether a real human work system will adopt the delegation mode, preserve appropriate accountability, and realize value once participation, trust, control preferences, and spillovers are endogenous. Delegation should therefore be separated from generic automation: automation names task execution by a system, while delegation names a human or organizational choice to grant execution authority to an agent acting on behalf of someone or some workflow.

[[2026-zhu-choose-your-agent]] gives the topic its first direct behavioral evidence. In three-person bargaining games, participants preferred the higher-control Advisor interface, but groups achieved the strongest welfare gains when they had access to an autonomous Delegate. The result matters because the underlying model capability was held constant: the performance difference came from the allocation of agency and execution rights, not simply from a better model.

The source also shows why delegation is a system-level design problem. Delegate non-users appeared to benefit from better offers entering the market, suggesting that autonomous agents can reshape the interaction environment even for humans who do not personally delegate. That makes agentic delegation different from a private productivity tool: in interdependent work, one worker's delegation can change the opportunity structure, risks, and norms facing everyone else.
...
- Relates to [[ai-adoption]] because realized value depends on whether users actually take up delegation when it is available, not just whether the delegate can outperform humans in an all-agent benchmark.
- Relates to [[ai-receptivity]] because users may prefer more controllable but lower-performing assistance even when delegation improves objective outcomes.
...
- Preference-performance misalignment cuts both ways: preserving user control may reduce welfare in some strategic settings, while forcing execution through a Delegate may create unacceptable accountability, trust, or learning costs in others.
