...
## Working the acquisition backlog

Triage sometimes judges a paper wiki-worthy without being able to obtain its text (paywall, bot
wall). Those records live in Drive `_triage/needs-acquisition.md`, regenerated by every daily triage
run, oldest first, with a link per row; the daily digest's last line carries the count. To bring one
in, download it yourself and drop the file into `_triage/wiki` under any name — the drain ingests it
and the row disappears once its DOI matches a wiki record. To give up on one, `reject <record id> —
<reason>` in #research-digest. Nothing retries automatically and no record is written from an
abstract alone (owner decision 2026-09-08).
