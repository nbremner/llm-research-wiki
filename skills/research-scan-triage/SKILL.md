...
| `wiki` + clear + no artifact | bounded rung-4 acquisition attempt (below); else "needs manual acquisition" in the digest **and** the record joins the needs-acquisition ledger (Drive `_triage/needs-acquisition.md`, regenerated by the applier every run; the digest's last line carries the count) until the owner drops a copy into `_triage/wiki` or rejects it |
...
7. **Deliver only the digest** (the applier prints it, ending with the `Acquisition backlog:` line — keep it) as your response — the cron job's delivery target
